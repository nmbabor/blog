...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $recentPost=Post::leftJoin('categories','posts.fk_category_id','categories.id')->leftJoin('users','posts.created_by','users.id')->where(['posts.status'=>1,'posts.is_pin'=>0])->where('posts.id','!=',$bigPost->id)->orderBy('posts.id','Desc')->select('posts.*','category_name','name')->simplePaginate(10);
        ...
        return view('frontend.index',compact('pinPost','recentPost','bigPost','popular','category','info'));
    }
