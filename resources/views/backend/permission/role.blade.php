                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            ...
                            @foreach($allData as $data)
                                <?php $label='<label class="col-md-3 permission_label"><input type="checkbox" name="permission_id[]" value="'.$data->id.'">'.$data->name.'</label>'; ?>
                                <?php foreach($permissionRole as $permission){?>
                                @if($permission->permission_id == $data->id)
                                 <?php $label='<label class="col-md-3 permission_label"><input type="checkbox" name="permission_id[]" value="'.$data->id.'" checked>'.$data->name.'</label>'; ?>
                                <?php break; ?>
                                @endif
                                <?}
                                echo $label;
